...
def save_df(df: pd.DataFrame, label: str = "blob") -> str:
    """Serialize df to parquet and store in Vercel Blob. Returns the public URL."""
    buf = io.BytesIO()
    df.to_parquet(buf, index=False)
    pathname = f"mm/{label}/{uuid.uuid4()}.parquet"
    resp = requests.put(
...
def load_df(url: str) -> pd.DataFrame:
    """Download a parquet blob from Vercel Blob and return as DataFrame."""
